...
router.get("/quote/random", function (req, res) {
  ...
  res.status(200).json({ quote: random.quote, member: random.member, info: random.info });
});
...
router.get("/api", function (req, res) {
  purpleCount++;
  res.status(200).json({
    message: "API создано в обучающих целях и не претендует на что-то выдающееся и уникальное в своём роде. API состоит из базы данных в формате JSON и самих запросов, на которых и стоится вся логика API. С уважением ваш Denchik797!",
    purpleCount,
  });
});
